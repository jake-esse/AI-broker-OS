...
def run_intake_test(email_path: Path) -> Dict[str, Any]:
    ...
    result = subprocess.run(
        ["python", "src/agents/intake/graph.py", str(email_path)],
        capture_output=True,
        text=True
    )
...
def run_missing_info_handler(email_path: Path) -> Dict[str, Any]:
    ...
    from src.agents.intake.missing_info_handler import process_email_with_intent
...
def test_email_classification():
    ...
    from src.services.email.classifier import classify_email_content, EmailIntent
...
def test_information_extraction():
    ...
    extracted = extract_missing_information(email_body, missing_fields)
    
    print(f"Missing fields requested: {missing_fields}")
    print(f"Extracted information: {extracted}")
    
    # Verify extraction
    assert "dest_zip" in extracted
    assert extracted["dest_zip"] == "30303"
    assert "weight_lb" in extracted
    assert extracted["weight_lb"] in [25000, "25000"]
